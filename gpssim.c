...
int Mmain(int argc, char *argv[],char *value[])
{
    char *valuee[] = { value[0], value[1], value[2], value[3], value[4], value[5], value[6],value[7],value[8] };

    argc = sizeof(valuee) / sizeof(valuee[0]);
    argv = valuee;
    ...
    while ((result=getopt(argc,argv,"e:u:x:g:c:l:o:s:b:T:t:d:iv"))!=-1)
    {
        ...
        case 's':
            ...
            if (samp_freq<1.0e6)
            {
                ...
                fprintf(fptr, "ERROR: Invalid sampling frequency.\n");
                //exit(1);
            }
            break;
        case 'b':
            ...
            if (data_format!=SC01 && data_format!=SC08 && data_format!=SC16)
            {
                ...
                fprintf(fptr,"ERROR: Invalid I/Q data format.\n");
               // exit(1);
            }
            break;
        case 'T':
        ...
        case 't':
                ...
                t0.hh<0 || t0.hh>23 || t0.mm<0 || t0.mm>59 || t0.sec<0.0 || t0.sec>=60.0)
            {
                ...
                fprintf(fptr,"ERROR: Invalid date and time.\n");
               // exit(1);
            }
            t0.sec = floor(t0.sec);
            date2gps(&t0, &g0);
    ...
    if (navfile[0]==0)
    {
        ...
        fprintf(fptr, "ERROR: GPS ephemeris file is not specified.\n");
       // exit(1);
    }
    ...
    if (duration<0.0 || (duration>((double)USER_MOTION_SIZE)/10.0 && !staticLocationMode) || (duration>STATIC_MAX_DURATION && staticLocationMode))
    {
        ...
        fprintf(fptr, "ERROR: Invalid duration.\n");
        //exit(1);
    }
    ...
    if (!staticLocationMode)
    {
        ...
        if (nmeaGGA==TRUE)
        {
            ...
            fprintf(stderr,"Using Nmea File.\n");
            fprintf(fptr, "Using Nmea File.\n");
        ...
        if (numd==-1)
        {
            ...
            fprintf(fptr,"ERROR: Failed to open user motion / NMEA GGA file.\n");
            //exit(1);
        }
        else if (numd==0)
        {
            fprintf(stderr, "ERROR: Failed to read user motion / NMEA GGA data.\n");
            fprintf(fptr,"ERROR: Failed to read user motion / NMEA GGA data.\n");
           // exit(1);
        }
    ...
    else
    {
        ...
        fprintf(fptr,"Using static location mode.\n");
...
*/
    ...
    if (neph==0)
    {
        ...
        fprintf(fptr,"ERROR: No ephemeris available.\n");
       // exit(1);
    }
    else if (neph==-1)
    {
        fprintf(stderr, "ERROR: ephemeris file not found.\n");
        fprintf(fptr,"ERROR: ephemeris file not found.\n");
       // exit(1);
    }
    ...
    if ((verb==TRUE)&&(ionoutc.vflg==TRUE))
    {
        ...
        fprintf(fptr,"  %12.3e %12.3e %12.3e %12.3e\n",
               ionoutc.alpha0, ionoutc.alpha1, ionoutc.alpha2, ionoutc.alpha3);
        fprintf(stderr, "  %12.3e %12.3e %12.3e %12.3e\n",
        ...
        fprintf(fptr,"   %19.11e %19.11e  %9d %9d\n",
               ionoutc.A0, ionoutc.A1, ionoutc.tot, ionoutc.wnt);
        fprintf(stderr, "%6d\n", ionoutc.dtls);
        fprintf(fptr,"%6d\n", ionoutc.dtls);
    }
    ...
    fprintf(fptr, "Start time = %4d/%02d/%02d,%02d:%02d:%02.0f (%d:%.0f)\n",
           t0.y, t0.m, t0.d, t0.hh, t0.mm, t0.sec, g0.week, g0.sec);
    fprintf(stderr, "Duration = %.1f [sec]\n", ((double)numd)/10.0);
    fprintf(fptr,"Duration = %.1f [sec]\n", ((double)numd)/10.0);
    ...
    if (ieph == -1)
    {
        ...
        fprintf(fptr,"ERROR: No current set of ephemerides has been found.\n");
       // exit(1);
    }
    ...
    if (iq_buff==NULL)
    {
        ...
        fprintf(fptr,"ERROR: Failed to allocate 16-bit I/Q buffer.\n");
        //exit(1);
    }
    ...
    if (data_format==SC08)
    {
        ...
        if (iq8_buff==NULL)
        {
            ...
            fprintf(fptr,"ERROR: Failed to allocate 8-bit I/Q buffer.\n");
           // exit(1);
        }
    }
    else if (data_format==SC01)
    {
        ...
        if (iq8_buff==NULL)
        {
            ...
            fprintf(fptr,"ERROR: Failed to allocate compressed 1-bit I/Q buffer.\n");
            //exit(1);
        }
    }
    ...
    if(strcmp("-", outfile)){
        if (NULL==(fp=fopen(outfile,"wb")))
        {
            ...
            fprintf(fptr, "ERROR: Failed to open output file.\n");
           // exit(1);
        }
    }else{
        fp = stdout;
    ...
    for(i=0; i<MAX_CHAN; i++)
    {
        ...
        fprintf(fptr, "%02d %6.1f %5.1f %11.1f %5.1f\n", chan[i].prn,
               chan[i].azel[0]*R2D, chan[i].azel[1]*R2D, chan[i].rho0.d, chan[i].rho0.iono_delay);
    }
    ...
    for (iumd=1; iumd<numd; iumd++)
    {
        ...
        // Update receiver time
        grx = incGpsTime(grx, 0.1);
        timehandel=subGpsTime(grx, g0);
        // Update time counter

        fprintf(stderr, "\rTime into run = %4.1f", subGpsTime(grx, g0));
        fprintf(fptr,"\rTime into run = %4.1f\n", subGpsTime(grx, g0));
        fflush(stdout);
    }
    ...
    fprintf(fptr,"Process time = %.1f [sec]\n", (double)(tend-tstart)/CLOCKS_PER_SEC);
    fclose(fptr);
    Sleep(1500);
    timehandel = 0.0;
    return(0);
